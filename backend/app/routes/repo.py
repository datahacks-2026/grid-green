...
def analyze_repo(
    ...
    total = 0
    # Repo scans fan out over many files; force local RAG per call instead of
    # mutating process-wide environment state while other requests may run.
    for f in files:
        code = f.content
        if f.path.lower().endswith(".ipynb"):
